...
const pool = new Pool({
  ...
  host:
    process.env.DB_HOST ||
    "ep-hidden-heart-a84r8c4i-pooler.eastus2.azure.neon.tech",
  database: process.env.DB_NAME || "neondb",
  password: process.env.DB_PASSWORD || "npg_OAPGCtyKX02E",
...
class AppointmentDB {
  async saveAppointment(appointmentData) {
    ...
    const result = await pool.query(
      "INSERT INTO appointments (date, title, time) VALUES ($1, $2, $3) RETURNING *",
      [date, title || "Appointment", time || "12:00 PM"]
    );
